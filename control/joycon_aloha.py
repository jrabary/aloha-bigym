...
class AlohaMocapControl:
    ...
    def joycon_control_l(self):
        ...
        print(f"acceleration pre gravity compensation: X={accel_vec[0]:.2f}, Y={accel_vec[1]:.2f}, Z={accel_vec[2]:.2f}")
        #TODO: account for gravity by taking the gravity magnitude and finding its values in the x y and z acis based on rotation 
        # (assuming starting rot_l is identity and had all gravity in the z axis, and starting rot_r is -identity and had all gravity in the z axis)
        # then subtracting those values from the accel_vec
        accel_vec = self.compensate_gravity(accel_vec, self.rot_l, self.gravity_magnitude_left)
        print(f"self rotational gravity left: {self.rot_l.as_matrix() @ np.array([0, 0, -self.gravity_magnitude_left])}")
        print(f"Acceleration: X={accel_vec[0]:.2f}, Y={accel_vec[1]:.2f}, Z={accel_vec[2]:.2f}")

        filtered_accel = self.high_pass_filter(accel_vec, self.previous_accel_l, self.previous_filtered_accel_l, self.accel_filter_alpha)
        self.previous_accel_l = accel_vec
        self.previous_filtered_accel_l = filtered_accel

        local_accel = np.linalg.inv(self.rot_l.as_matrix()) @ filtered_accel

        # Convert acceleration to world frame
        world_accel = self.rot_l.as_matrix() @ local_accel

        # Create translation matrix from acceleration
        translation_matrix = self.accel_to_translation_matrix(world_accel, self.dt)

        # Update target position using matrix multiplication
        target_homogeneous = np.append(self.target_l, 1)
        new_target_homogeneous = translation_matrix @ target_homogeneous
        self.target_l = new_target_homogeneous[:3]
    ...
    def joycon_control_r(self):
        ...
        accel = status['accel']
        rotation = status['gyro']
        button = status['buttons']['right']['zr']
        ...
        print(f"acceleration pre gravity compensation: X={accel_vec[0]:.2f}, Y={accel_vec[1]:.2f}, Z={accel_vec[2]:.2f}")
        accel_vec = self.compensate_gravity(accel_vec, self.rot_r, self.gravity_magnitude_right)
        print(f"self rotational gravity right: {self.rot_r.as_matrix() @ np.array([0, 0, -self.gravity_magnitude_right])}")
        print(f"Acceleration: X={accel_vec[0]:.2f}, Y={accel_vec[1]:.2f}, Z={accel_vec[2]:.2f}")

        filtered_accel = self.high_pass_filter(accel_vec, self.previous_accel_r, self.previous_filtered_accel_r, self.accel_filter_alpha)
        self.previous_accel_r = accel_vec
        self.previous_filtered_accel_r = filtered_accel

        local_accel = np.linalg.inv(self.rot_r.as_matrix()) @ filtered_accel

        # Convert acceleration to world frame
        world_accel = self.rot_r.as_matrix() @ local_accel
        
        # Create translation matrix from acceleration
        translation_matrix = self.accel_to_translation_matrix(world_accel, self.dt)

        # Update target position using matrix multiplication
        target_homogeneous = np.append(self.target_r, 1)
        new_target_homogeneous = translation_matrix @ target_homogeneous
        self.target_r = new_target_homogeneous[:3]
