...
class RobotFloatingBase:
    ...
    def __init__(
        ...
        self._pelvis = pelvis
        self._mojo = mojo
        self._fixed_position = np.array(self._config.offset_position).astype(np.float32)

        # Add a fixed joint to the pelvis
        self._pelvis.mjcf.add(
            "joint",
            type="fixed",
        ...
        self._set_position(self._fixed_position)
    ...
    def reset(self, position: np.ndarray, quaternion: np.ndarray):
        """Set position and orientation of the fixed base."""
        self._set_position(position)
        self._set_quaternion(quaternion)
        if self._animated_legs:
            self._animated_legs.step(self._pelvis_z, False)
    ...
    def _set_position(self, position: np.ndarray):
        pelvis = self._mojo.physics.bind(self._pelvis.mjcf)
        pelvis.pos[:] = position + self._fixed_position

    def _set_quaternion(self, quaternion: np.ndarray):
        pelvis = self._mojo.physics.bind(self._pelvis.mjcf)
        pelvis.quat[:] = quaternion
    ...
    def is_target_reached(self) -> bool:
        ...
        return True  # Always true for fixed base

    @property
    def dof_amount(self) -> int:
        """Get number of actuated DOF."""
        return 0  # No DOFs for fixed base

    @property
    def qpos(self) -> np.ndarray:
        """Get positions of actuated joints."""
        return np.array([], dtype=np.float32)  # Empty array for fixed base
    ...
    def all_actuators(self) -> list[mjcf.Element]:
        """Get all actuators."""
        return []  # No actuators for fixed base

    def get_action_bounds(self) -> list[tuple[float, float]]:
        """Get action bounds of all actuators."""
        return []  # No action bounds for fixed base

    def set_control(self, control: np.ndarray):
        """Set control of all actuators."""
        pass  # No control to set for fixed base