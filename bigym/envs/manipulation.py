...
class FlipCutlery(_ManipulationEnv):
    ...
    def _on_reset(self):
        ...
        self.spoon.body.set_position(spawn_point + self._SPOON_OFFSET)
        self.spoon.body.set_quaternion(self._SPOON_QUAT.elements)
...
class StackBlocks(BiGymEnv):
    ...
    _NUM_BLOCKS = 3
    _BLOCKS_POS = np.array([0.1, 0, 0.77])
    _BLOCKS_POS_EXTENTS = np.array([0.2, 0.2])
    _BLOCKS_STEP = 0.15
    ...
    _TARGET_POS = np.array([-0.1, 0, 0.77])
