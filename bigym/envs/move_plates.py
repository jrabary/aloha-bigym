...
RACK_POSITION_LEFT = np.array([0.2, 0.05, 0.75])
RACK_POSITION_RIGHT = np.array([-0.2, 0.05, 0.75])
...
class _MovePlatesEnv(BiGymEnv, ABC):
    ...
    def _on_reset(self):
        ...
        for site, plate in zip(sites, self.plates):
            plate.body.set_position(site.get_position() + PLATE_OFFSET_POS)
            quat = Quaternion(site.get_quaternion())
            quat *= PLATE_OFFSET_ROT
            plate.body.set_quaternion(quat.elements)
