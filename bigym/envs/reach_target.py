"""Set of reach target tasks for ALOHA Robot."""
from abc import ABC
...
class _ReachTargetEnv(BiGymEnv, ABC):
    """Base reach target environment for ALOHA."""

    TARGET_CONFIGS = [
        TargetConfig(
            target_hands=[HandSide.RIGHT],
            reset_position=np.array([0.5, -0.2, 1]),  # Adjusted for right arm
            color_default=np.array([0.3, 0, 0, 1]),
            color_highlight=np.array([1, 0, 0, 1]),
        ),
        TargetConfig(
            target_hands=[HandSide.LEFT],
            reset_position=np.array([0.5, 0.2, 1]),  # Adjusted for left arm
...
class ReachTargetSingle(ReachTarget):
    """Reach the target with the right wrist."""
...
class ReachTargetDual(_ReachTargetEnv):
    ...
    TARGET_CONFIGS = [
        TargetConfig(
            target_hands=[HandSide.LEFT],
            reset_position=np.array([-0.2, 0.2, 1]),
            color_default=np.array([0.3, 0, 0, 1]),
            color_highlight=np.array([1, 0, 0, 1]),
        ),
        TargetConfig(
            target_hands=[HandSide.RIGHT],
            reset_position=np.array([0.1, 0, 1]),
            color_default=np.array([0, 0.3, 0, 1]),
            color_highlight=np.array([0, 1, 0, 1]),
        ),
    ]
